...
// Mini web-app that auto-copies the KPay number to the clipboard.
// (Plain inline buttons can never touch the clipboard — Telegram rule —
// so the button opens this tiny page, which copies on load.)
const COPY_PAGE_URL =
...
export async function handleCallback(send, query) {
  ...
  if (data === "about") {
    ...
    // Copy button opens the mini web-app, which auto-copies the number
    ...
    await editVipScreen(
      ...
      VIP_PAY_TEXT,
      {
        inline_keyboard: [
          [
            { text: "📋 နံပါတ်ကော်ပီယူရန်", web_app: { url: COPY_PAGE_URL } },
            { text: "📤 စလစ်ပို့ရန်", callback_data: "send_slip" },
          ],
          [{ text: "⬅️ Back", callback_data: "vip_back" }],
        ],
