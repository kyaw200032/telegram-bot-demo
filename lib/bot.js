...
const VIP_PAY_TEXT =
  ...
  "👤 Name: Ma Pan Ei Phyu\n" +
  "\n" +
  "👆 နံပါတ်ကို တစ်ချက်နှိပ်လိုက်ရင် ကူးယူပြီးသား ✅\n" +
  "\n" +
...
export async function handleCallback(send, query) {
  ...
  if (data === "about") {
    ...
    // The KPay number is shown as a code block: one tap on it copies it.
    // (No bot can write to the phone clipboard directly - Telegram rule.)
    await editVipScreen(send, query, VIP_PAY_TEXT, vipBackKeyboard(), "Markdown");
