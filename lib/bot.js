...
export function verifySlipText(text, strict) {
  ...
  // 1) amount: 10,000 MMK
  const amountOk = strict
    ? /10,?000(\.00)?/.test(t)
    : digits.includes("10000");
  if (!amountOk) reasons.push("ပမာဏ 10000 မတွေ့");

  // 2) receiver account
  let accountOk = false;
  if (strict) {
    accountOk = t.includes(KPAY_NUMBER);
  } else {
    const runs = (t.replace(/\D/g, " ").match(/\d{9,13}/g) || []);
    accountOk = runs.some((r) => levenshtein(r, KPAY_NUMBER) <= 3);
  }
  if (!accountOk) reasons.push("လက်ခံအကောင့် မတွေ့");

  // 3) receiver name
  if (!/ma\s*pan\s*ei\s*phyu/i.test(t)) reasons.push("လက်ခံသူအမည် မတွေ့");
  ...
  // 5) transaction id present (8+ chars, letters + digits, word-bound)
  if (!/\b(?=[A-Z0-9-]*[A-Z])[A-Z0-9-]*\d[A-Z0-9-]{6,}\b/i.test(t))
    reasons.push("Transaction ID မတွေ့");
