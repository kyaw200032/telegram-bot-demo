...
async function editVipScreen(send, query, text, keyboard) {
...
export async function handleCallback(send, query) {
  ...
  if (data === "about") {
    ...
    await editVipScreen(
      ...
      VIP_PAY_TEXT,
      vipBackKeyboard([
        [{ text: "📋 နံပါတ်ကော်ပီယူရန်", callback_data: "vip_copy" }],
      ])
    );
  } else if (data === "vip_copy") {
    // Telegram buttons can't write to the clipboard directly — the standard
    // trick: send the number as a code block, one tap on it copies it.
    await send("sendMessage", {
      chat_id: chatId,
      text: `📋 နံပါတ်ကို တစ်ချက်နှிக်လိုက်ရင် ကူးယူပြီးသား ✅\n\n\`${KPAY_NUMBER}\`\n👤 ${KPAY_NAME}`,
      parse_mode: "Markdown",
    });
