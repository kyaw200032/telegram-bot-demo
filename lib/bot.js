...
// flow. Config: sheet settings supabase_url + supabase_anon_key, and the
// Vercel env var VIP_SYNC_SECRET (kept out of the sheet and the repo).
// ---------------------------------------------------------------------------
async function syncVipToWebsite(send, telegramId, username) {
  try {
    const s = await getSettings();
    const base = (s.supabase_url || "").replace(/\/+$/, "");
    const key = s.supabase_anon_key || "";
    const secret = process.env.VIP_SYNC_SECRET || "";
    if (!base || !key || !secret) {
      console.error(
        "vip sync skipped: missing supabase_url / supabase_anon_key / VIP_SYNC_SECRET"
      );
      return;
    }
    const uname = String(username || "").replace(/^@/, "").toLowerCase();
    const res = await fetch(`${base}/rest/v1/rpc/record_vip_purchase`, {
      method: "POST",
      headers: {
        apikey: key,
...
async function getRaffleData() {
  ...
  try {
    const s = await getSettings();
    const rows = await fetchCSV(raffleTabUrl(s.raffle_tab || "Form_Responses2"), true);
    if (rows && rows.length > 1) {
      const head = rows[0].map((h) => (h || "").trim().toLowerCase());
...
export async function handleRaffleCallback(send, query, data) {
  ...
  if (data === "raffle_ss") {
    const { open: raffleOpen } = await getRaffleData();
    if (!raffleOpen) {
      await send("sendMessage", { chat_id: chatId, text: "🚫 မဲစာရင်းပိတ်ပါပြီ" });
      return;
    }
...
async function handleRafflePhoto(send, msg) {
  ...
  if (!raffleOpen) {
    rafflePending.delete(userId);
    await send("sendMessage", { chat_id: msg.chat.id, text: "🚫 မဲစာရင်းပိတ်ပါပြီ" });
    return;
  }
  const username = msg.from?.username ? `@${msg.from.username}` : "";
  let text = "";
